    ...
    public function tree($tree, $class) {
		...
		if( isset($tree) && count($tree) > 0 ){
		    ...
		    foreach($tree as $item){
		      if($prevLevel > 0 && $item->getLevel()  == $prevLevel)  echo '</li>';
		      if($item->getLevel() > $prevLevel)  echo '<ol>';
		      elseif ($item->getLevel()  < $prevLevel) echo str_repeat('</ol></li>', $prevLevel - $item->getLevel() );
		      ...
		      $prevLevel = $item->getLevel() ; $is_first = false;
		    }
